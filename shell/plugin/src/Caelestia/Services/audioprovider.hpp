...
protected:
    ...
    [[nodiscard]] static bool isSilent(const Sample* samples, std::size_t count) {
        return count == 0 || std::all_of(samples, samples + count, [](Sample sample) { return sample == Sample(0); });
    }
